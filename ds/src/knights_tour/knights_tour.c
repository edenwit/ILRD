...
static int IsStepOccupied(char position, bit_arr_t chess_board)
{
    return (BitArrGetVal(chess_board[position]));
}

static char GetNextAvalableStep(char position, bit_arr_t chess_board)
{    
    for (i = 0; (i < BOARD_AXIS) && (-1 != lut[(size_t)curr_position][i]) && 
         BitArrGetVal(chess_board, lut[(size_t)curr_position][i]); ++i)
    {
        ;
    }
    ...
    return (lut[(size_t)curr_position][i]);
}
