...
int DLLIsEmpty(const d_list_t *list)
{
	...
	return ((list->head_dummy.next) == (d_list_node_t *)&(list->tail_dummy));
}
...
d_list_iter_t DLLInsert(d_list_iter_t where, void *data)
{
	...
	node->data = data;
	node->next = where;
	node->prev = DLLPrev(where);
	
	where->prev = node;
	where = DLLPrev(DLLPrev(where));
	where->next = node;
...
d_list_iter_t DLLRemove(d_list_iter_t iter)
{
	...
	d_list_iter_t temp_prev = NULL;
	
	assert(NULL != iter->next);

	temp_next = DLLNext(iter);
	temp_prev = DLLPrev(iter);
